...
import type { VideoMeta, VideoProgressData } from "@/lib/types";
...
export default function VideoPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const m = await apiGet<VideoMeta>(`/videos/${videoId}`);
        if (cancelled) return;
        setMeta(m);
        setVideo({
          currentVideoId: m.id,
        ...
        if (!m.locked) {
          const p = await apiGet<VideoProgressData>(
          ...
          setVideo({
            ...
            currentTime: p.last_position_seconds,
          });
        } else {
          setProgress(null);
        }
    ...
    >
      ...
      {isLocked && !isCompleted ? (
        <div className="aspect-video rounded-xl border border-border bg-surface flex flex-col items-center justify-center text-center p-8">
          ...
          <h2 className="font-heading font-semibold text-lg">Video locked</h2>
      ...
      <div className="flex items-center justify-between border-t border-border pt-4">
        <div>
          {meta.previous_video_id ? (
            ...
            >
              <Link
                href={`/subjects/${subjectId}/video/${meta.previous_video_id}`}
              >
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          {meta.next_video_id && isCompleted ? (
            ...
            >
              <Link
                href={`/subjects/${subjectId}/video/${meta.next_video_id}`}
              >
