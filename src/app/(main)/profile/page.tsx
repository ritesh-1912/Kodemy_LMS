...
function ProfileContent() {
  ...
  const [subjects, setSubjects] = useState<(SubjectItem & SubjectProgress)[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const listing = await apiGet<{ subjects: SubjectItem[] }>(
          "/api/subjects?page=1&pageSize=50"
        );
  ...
  return (
    <div className="container max-w-2xl py-10 space-y-8">
      ...
      <div>
        <h2 className="font-heading text-lg font-semibold mb-4">Your progress</h2>
        {loading ? (
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  {s.last_video_id && (
                    <Button asChild size="sm" variant="ghost" className="shrink-0 text-primary hover:text-primary">
                      <Link href={`/subjects/${s.id}/${s.last_video_id}`}>
                        Continue
                      </Link>
                    </Button>
                  )}
